...
<span id="hookscript"></span>the hookscript that stops a docker VM starting without cephFS is on [cephFS start guards](cephfs-start-guards.md).
...
<span id="docker-data-guard"></span>once the data has moved (below), add the check that stops docker starting on a missing or empty mount, see [cephFS start guards](cephfs-start-guards.md#docker-data-guard).
...
Update as of 2026.09.24: check each moved volume really is the cephFS bind, see
[troubleshooting](../docker/troubleshooting.md#a-volume-moved-to-cephfs-is-still-on-local-disk).
...
Update as of 2026.09.24: the cephFS is backed up hourly to PBS now, see [cephFS backups](../backups/cephfs.md).
