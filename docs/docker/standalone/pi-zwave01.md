---
title: "pi-zwave01"
---

# pi-zwave01

a raspberry pi 4 holding the usb radios for home automation. the [raspberry pi section](../../raspberry-pi/index.md) covers the box; its docker side is in [docker and stacks](../../raspberry-pi/stacks.md).

## stacks from git

- [zwave-js-ui](../../raspberry-pi/stacks.md#zwave-js-ui) - the z-wave radio
- [ser2net](../../raspberry-pi/stacks.md#ser2net) - the thread radio, for home assistant's border router
- [dozzle agent](../../monitoring/dozzle.md) and [glances](../../monitoring/glances.md)
