...
//// details | compose.yml, 117 lines, 2 notes
    type: example

each <span class="file-note-marker" title="note marker"></span> in the code opens a note on that line. [download compose.yml](../files/swarm/wordpress2025/compose.yml){ download="wordpress2025-compose.yml" }
{: .file-legend }
...
1.  binary logs for two days instead of mysql 8's thirty. they are only for replaying changes
    after a restore, see [the hourly dump](#the-hourly-dump).
2.  the hourly dump: the db service's image, running `db-dump.sh` instead of mysqld. it dumps
    `wordpressdb` at start and at :50, see [the hourly dump](#the-hourly-dump).
